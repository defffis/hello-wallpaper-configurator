...
- Live Photo beta: JPEG получает Apple MakerNote `0x0011`, MOV — совпадающий `com.apple.quicktime.content.identifier`; на iPhone пара передаётся вместе через Web Share, на компьютере сохраняется ZIP с парой, manifest и инструкцией.
...
**iPhone:** сначала можно использовать **«Экспорт Live Photo · beta»**. Браузер формирует JPG + MOV с общим Apple Content Identifier и, если Web Share позволяет, передаёт оба файла одновременно. Если «Фото» не объединит пару или iOS не разрешит использовать её как живые обои, экспортируйте обычный MP4 и преобразуйте его в Live Photo сторонним приложением. Затем добавьте Live Photo на экран блокировки и включите воспроизведение. [Инструкция Apple](https://support.apple.com/en-us/120734).
