...
- Liquid Glass: преломление фона внутри букв, прозрачный оттенок, светлый и тёмный края, мягкая тень; регулируемый объём.
...
- Live Photo `.livp`: JPEG получает Apple MakerNote `0x0011`, MOV — совпадающий `com.apple.quicktime.content.identifier` и отдельный `mebx` timed metadata track `com.apple.quicktime.still-image-time`. Пара упаковывается в единый `.livp`.
...
Видео содержит одно появление и заканчивается готовой надписью. Файл не задаёт политику повторов или системный триггер — это настройки приложения обоев.

**iPhone:** используйте **«Скачать Live Photo (.livp)»**. Файл содержит корректно связанную пару JPG + MOV. Важно: Safari/Web Share не имеет PhotoKit API для создания одного `PHAsset` из ресурсов `.photo` + `.pairedVideo`, поэтому прямое «Сохранить в Фото» распакованной пары создаёт отдельные фото и видео. Сохраняйте `.livp` в «Файлы»/совместимое приложение. Для гарантированного прямого добавления в медиатеку нужен нативный импорт через `PHAssetCreationRequest`. После появления Live Photo в «Фото» добавьте его на экран блокировки. [Инструкция Apple](https://support.apple.com/en-us/120734).
...
Целевые браузеры: актуальные Chrome, Edge, Firefox, Safari, Chrome Android и Safari iOS. Подробности фактически выполненных проверок — в `TESTING.md`; поведение прямого Web Share уже проверено на физическом iPhone и признано неподходящим для создания единого Live Photo.
