...
- Live Photo экспортируется как связанная пара `.HEIC` + `.MOV` с одинаковым `com.apple.quicktime.content.identifier`; HEIC получает Apple MakerNote tag `0x0011`, MOV — timed metadata `com.apple.quicktime.still-image-time` и `com.apple.quicktime.live-photo-still-image-transform`.
...
Обычный MP4/WebM содержит одно появление и заканчивается готовой надписью. Live Photo использует отдельный профиль около 1 секунды: появление завершается в первой половине, затем финальный кадр удерживается без обратного движения.

**iPhone:** используйте **«Скачать Live Photo (HEIC + MOV)»**. На iPhone сайт передаёт оба связанных файла через Share Sheet; выберите PhotoSync и импортируйте пару как один Live Photo. Если вместо этого скачан ZIP, распакуйте его в «Файлах» и передайте `.HEIC` + `.MOV` вместе. После появления Live Photo в «Фото» добавьте его на экран блокировки и включите кнопку воспроизведения. [Инструкция Apple](https://support.apple.com/en-us/120734).
